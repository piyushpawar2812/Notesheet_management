...
def dashboard(request):
    ...
    from systemapp.models import Department, NoteRemark, NoteSheet
    from django.db.models import Q

    accessible_notes_qs = NoteSheet.objects.filter(
        Q(created_by=user)
        | Q(forwarded_to=user)
        | Q(remarks__created_by=user)
        | Q(remarks__forwarded_to=user)
    ).select_related('department', 'created_by', 'forwarded_to').distinct()

    selected_department = request.GET.get('department', '').strip()
    notes_list = accessible_notes_qs
    if selected_department:
        notes_list = notes_list.filter(department_id=selected_department)
    ...
    context = {
        "accessible_notes": accessible_notes_qs.count(),
        "inbox_count": NoteSheet.objects.filter(forwarded_to=user).count(),
        "draft_count": NoteSheet.objects.filter(created_by=user, forwarded_to__isnull=True).count(),
        "sent_count": NoteRemark.objects.filter(created_by=user, action='FORWARDED').count(),
